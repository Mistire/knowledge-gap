...
A colleague built an ORPO judge on Qwen 2.5 that outputs a structured JSON verdict on every B2B sales email. Her `scoring_evaluator.py` parses that verdict with `json.loads()`. About 2% of the time, the JSON is malformed and the pipeline crashes. Her system prompt says "Respond with JSON only." She wanted to know: if she switched to native function-calling, would that fix it — and what would be mechanically different?
...
The 2% malformed rate is the tail of this distribution. It is not a bug in the prompt. It is the correct behavior of a statistical model with no structural constraint on its output.
...
A judge that always outputs a verdict is not choosing between tools. That use case does not need function-calling — it needs **structured outputs**, which apply the same constrained decoding mechanism to regular completions.

OpenAI's `response_format: {"type": "json_schema", "json_schema": {...}}` does this. Libraries like [Outlines](https://github.com/dottxt-ai/outlines) and lm-format-enforcer do it for open models. The mechanism is identical to function-calling's constraint layer: logit masking at each token position based on a JSON schema.

The difference in plain terms:

- **Prompt engineering** — full vocabulary at every token step, nonzero malformed rate
- **Native function-calling** — logit masking per schema at every step, zero malformed rate by construction
- **Structured outputs** — logit masking per schema at every step, zero malformed rate by construction

Function-calling is for agents that choose which tool to invoke. Structured outputs are for models that always return a fixed schema. Both use the same underlying constraint mechanism; they differ in when they apply it.

---

## Why Fine-Tuning Alone Is Not Enough

Qwen 2.5 was fine-tuned extensively on JSON output. Shouldn't that be enough?
...
Add a JSON schema matching your judge's output format and pass it as `response_format` when calling the model. The model's output will then always be a valid, parseable JSON object. The `json.loads()` call will never raise again — not because the model got better at formatting, but because the generation process no longer allows malformed output.
...
**Sources**
- Willard & Louf (2023) — [Efficient Guided Generation for Large Language Models](https://arxiv.org/abs/2307.09702) — the foundational paper on grammar-constrained decoding
- OpenAI — [Structured Outputs documentation](https://platform.openai.com/docs/guides/structured-outputs)
- [Outlines](https://github.com/dottxt-ai/outlines) — open-source constrained decoding for local models
